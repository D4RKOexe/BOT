...
const OWNER           = "51913454004@s.whatsapp.net";
  ...
  sock.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      try {
        ...
        // ─── Respuesta a mención directa o @todos en grupos ──────────────────
        if (isGroup && body && !body.startsWith(CONFIG.prefix)) {
          ...
          try {
            ...
            const esMencionadoDirecto = botLid
              ? menciones.some(m => m === botLid || m.includes(botNum))
              : menciones.some(m => m.includes(botNum));

            // @todos: se menciona a todos los participantes del grupo
            const esTodosOGran = menciones.length >= metadata.participants.length - 1 && menciones.length > 1;

            const esRespuesta = participantQuoted &&
              (participantQuoted.includes(botNum) || participantQuoted === botLid);

            fueMenucionado = esMencionadoDirecto || esTodosOGran || esRespuesta;

          } catch (e) {
            const esMencionadoDirecto = menciones.some(m => m.includes(botNum));
            const esRespuesta         = participantQuoted.includes(botNum);
            fueMenucionado = esMencionadoDirecto || esRespuesta;
            console.error("[MENCION GRUPO ERROR]", e.message);
          }

          if (fueMenucionado) {
            await reply(sock, jid, `⚠️ GR ya fue informado de esto`, msg);

            // Notificar al owner por privado
            try {
              let nombreGrupo = jid;
              try {
                const meta = await sock.groupMetadata(jid);
                nombreGrupo = meta.subject || jid;
              } catch {}

              const quien   = msg.pushName || sender.split("@")[0];
              const textoMsg = body.replace(/@\d+/g, "").trim() || "(sin texto)";
